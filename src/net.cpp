...
// Copyright (c) 2009-2012 The sector Core developers
...
void ThreadMapPort2(void* parg)
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "Sector " + FormatFullVersion();
#ifndef UPNPDISCOVER_SUCCESS
        /* miniupnpc 1.5 */
...
// testnet dns seed begins with 't', all else are Sector dns seeds.
static const char *strDNSSeed[][2] = {
    {"164.132.238.113", "164.132.238.113"},
    {"164.132.238.114", "164.132.238.114"},
    {"sector.sect.tech", "sector.sect.tech"},
};
...
void ThreadDNSAddressSeed2(void* parg)
{
    ...
    if (true /*!fTestNet*/)  // Sector enables dns seeding with testnet too
    {
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer.  Sector is probably already running."), addrBind.ToString().c_str());
        else
...
void StartNode(void* parg)
{
    ...
    Generatesectors(GetBoolArg("-gen", false), pwalletMain);

    // Sector: mint proof-of-stake blocks in the background
...
bool StopNode()
{
    ...
    if (vnThreadsRunning[THREAD_MINER] > 0) printf("ThreadsectorMiner still running\n");
