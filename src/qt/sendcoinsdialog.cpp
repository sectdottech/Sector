    ...
    model(0)
{
    ...
    ui->lineEditCoinControlChange->setPlaceholderText(tr("Enter a Sector address"));
#endif
    ...
    // Coin Control
    ui->lineEditCoinControlChange->setFont(GUIUtil::sectorAddressFont());
...
void SendCoinsDialog::on_sendButton_clicked()
{
    ...
    {
        formatted.append(tr("<b>%1</b> to %2 (%3)").arg(sectorUnits::formatWithUnit(sectorUnits::BTC, rcp.amount), Qt::escape(rcp.label), rcp.address));
    }
    ...
    case WalletModel::AmountWithFeeExceedsBalance:
        QMessageBox::warning(this, tr("Send Coins"),
            ...
            arg(sectorUnits::formatWithUnit(sectorUnits::BTC, sendstatus.fee)),
            QMessageBox::Ok, QMessageBox::Ok);
        break;
...
void SendCoinsDialog::handleURI(const QString &uri)
{
    ...
    if(!GUIUtil::parsesectorURI(uri, &rv))
    {
        return;
    }
    pasteEntry(rv);
...
void SendCoinsDialog::setBalance(qint64 balance, qint64 stake, qint64 unconfirmedBalance)
{
    ...
    ui->labelBalance->setText(sectorUnits::formatWithUnit(unit, balance));
}
...
void SendCoinsDialog::updateDisplayUnit()
{
    if(model && model->getOptionsModel())
    {
        ...
        ui->labelBalance->setText(sectorUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), model->getBalance()));
    }
}
...
void SendCoinsDialog::coinControlChangeChecked(int state)
{
    ...
    {
        if (state == Qt::Checked)
            CoinControlDialog::coinControl->destChange = CsectorAddress(ui->lineEditCoinControlChange->text().toStdString()).Get();
        else
...
void SendCoinsDialog::coinControlChangeEdited(const QString& text)
{
    if (model && model->getAddressTableModel())
    {
        ...
        CsectorAddress addr = CsectorAddress(text.toStdString());
        ...
        {
            ui->labelCoinControlChangeLabel->setText(tr("Warning: Invalid sector address"));
        }
        else // Valid address
        {
            CPubKey pubkey;
