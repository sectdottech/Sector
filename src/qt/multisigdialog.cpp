...
void MultisigDialog::on_createAddressButton_clicked()
{
    ...
    CsectorAddress address(scriptID);
...
void MultisigDialog::on_saveMultisigAddressButton_clicked()
{
    ...
    if(!wallet->HaveCScript(scriptID))
        wallet->AddCScript(script);
    if(!wallet->mapAddressBook.count(CsectorAddress(address).Get()))
        wallet->SetAddressBookName(CsectorAddress(address).Get(), label);
}
    ...
    for(int i = 0; i < ui->outputs->count(); i++)
    {
        ...
        if(entry)
        {
            if(entry->validate())
            {
                ...
                CsectorAddress address(recipient.address.toStdString());
