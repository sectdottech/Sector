...
void ScriptPubKeyToJSON(const CScript& scriptPubKey, Object& out)
{
    ...
    BOOST_FOREACH(const CTxDestination& addr, addresses)
        a.push_back(CsectorAddress(addr).ToString());
...
void TxToJSON(const CTransaction& tx, Object& txdata)
{
    ...
    BOOST_FOREACH(const CTxOut& txout, tx.vout)
    {
        ...
        if (ExtractDestinations(txout.scriptPubKey, txtype, addresses, nRequired))
        {
            ...
            Array addrs;
            BOOST_FOREACH(const CTxDestination& addr, addresses)
                addrs.push_back(CsectorAddress(addr).ToString());
...
Value setgenerate(const Array& params, bool fHelp)
{
    ...
    Generatesectors(fGenerate, pwalletMain);
    return Value::null;
}
...
// Sector: get network Gh/s estimate
...
Value getnewaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "Returns a new Sector address for receiving payments.  "
    ...
    return CsectorAddress(keyID).ToString();
}


CsectorAddress GetAccountAddress(string strAccount, bool bForceNew=false)
{
    ...
    return CsectorAddress(account.vchPubKey.GetID());
}
...
Value getaccountaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "Returns the current Sector address for receiving payments to this account.");
...
Value setaccount(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "setaccount <Sectoraddress> <account>\n"
            "Sets the account associated with the given address.");

    CsectorAddress address(params[0].get_str());
    if (!address.IsValid())
        throw JSONRPCError(-5, "Invalid Sector address");
...
Value getaddressesbyaccount(const Array& params, bool fHelp)
{
    ...
    Array ret;
    BOOST_FOREACH(const PAIRTYPE(CsectorAddress, string)& item, pwalletMain->mapAddressBook)
    {
        const CsectorAddress& address = item.first;
...
Value sendtoaddress(const Array& params, bool fHelp)
{
    if (pwalletMain->IsCrypted() && (fHelp || params.size() < 2 || params.size() > 4))
        throw runtime_error(
            "sendtoaddress <Sectoraddress> <amount> [comment] [comment-to]\n"
...
Value signmessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw runtime_error(
            "signmessage <Sectoraddress> <message>\n"
    ...
    CsectorAddress addr(strAddress);
    if (!addr.IsValid())
...
Value verifymessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage <Sectoraddress> <signature> <message>\n"
...
Value getreceivedbyaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "getreceivedbyaddress <Sectoraddress> [minconf=1]\n"
            "Returns the total amount received by <Sectoraddress> in transactions with at least [minconf] confirmations.");

    // sector address
    CsectorAddress address = CsectorAddress(params[0].get_str());
    CScript scriptPubKey;
    if (!address.IsValid())
        throw JSONRPCError(-5, "Invalid Sector address");
...
Value sendfrom(const Array& params, bool fHelp)
{
    if (pwalletMain->IsCrypted() && (fHelp || params.size() < 3 || params.size() > 6))
        throw runtime_error(
            "sendfrom <fromaccount> <toSectoraddress> <amount> [minconf=1] [comment] [comment-to]\n"
            "<amount> is a real and is rounded to the nearest 0.000001\n"
            "requires wallet passphrase to be set with walletpassphrase first");
    if (!pwalletMain->IsCrypted() && (fHelp || params.size() < 3 || params.size() > 6))
        throw runtime_error(
    ...
    string strAccount = AccountFromValue(params[0]);
    CsectorAddress address(params[1].get_str());
    if (!address.IsValid())
        throw JSONRPCError(-5, "Invalid Sector address");
    int64 nAmount = AmountFromValue(params[2]);
    if (nAmount < MIN_TXOUT_AMOUNT)
...
Value sendmany(const Array& params, bool fHelp)
{
    ...
    set<CsectorAddress> setAddress;
    vector<pair<CScript, int64> > vecSend;

    int64 totalAmount = 0;
    BOOST_FOREACH(const Pair& s, sendTo)
    {
        CsectorAddress address(s.name_);
        if (!address.IsValid())
            throw JSONRPCError(-5, string("Invalid Sector address:")+s.name_);
...
Value ListReceived(const Array& params, bool fByAccounts)
{
    ...
    // Tally
    map<CsectorAddress, tallyitem> mapTally;
    for (map<uint256, CWalletTx>::iterator it = pwalletMain->mapWallet.begin(); it != pwalletMain->mapWallet.end(); ++it)
    {
        const CWalletTx& wtx = (*it).second;
    ...
    BOOST_FOREACH(const PAIRTYPE(CsectorAddress, string)& item, pwalletMain->mapAddressBook)
    {
        ...
        map<CsectorAddress, tallyitem>::iterator it = mapTally.find(address);
...
void ListTransactions(const CWalletTx& wtx, const string& strAccount, int nMinDepth, bool fLong, Array& ret)
{
    ...
    if ((!listSent.empty() || nFee != 0) && (fAllAccounts || strAccount == strSentAccount))
    {
        BOOST_FOREACH(const PAIRTYPE(CTxDestination, int64)& s, listSent)
        {
            ...
            entry.push_back(Pair("address", CsectorAddress(s.first).ToString()));
    ...
    if (listReceived.size() > 0 && wtx.GetDepthInMainChain() >= nMinDepth)
    {
        BOOST_FOREACH(const PAIRTYPE(CTxDestination, int64)& r, listReceived)
        {
            ...
            if (fAllAccounts || (account == strAccount))
            {
                ...
                entry.push_back(Pair("address", CsectorAddress(r.first).ToString()));
...
Value walletpassphrase(const Array& params, bool fHelp)
{
    ...
    // Sector: if user OS account compromised prevent trivial sendmoney commands
    if (params.size() > 2)
...
Value encryptwallet(const Array& params, bool fHelp)
{
    ...
    StartShutdown();
    return "wallet encrypted; Sector server stopping, restart to run with encrypted wallet";
}
...
Value validateaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress <Sectoraddress>\n"
            "Return information about <Sectoraddress>.");
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "Sector is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "Sector is downloading blocks...");
...
Value getblocktemplate(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "See https://en.sector.it/wiki/BIP_0022 for full specification.");
...
// Sector: get information of sync-checkpoint
...
// Sector: reserve balance from being staked for network protection
...
// Sector: make a public-private key pair
...
CScript _createmultisig(const Array& params)
{
    ...
    for (unsigned int i = 0; i < keys.size(); i++)
    {
        ...
        // Case 1: sector address and we have full public key:
        CsectorAddress address(ks);
...
Value addmultisigaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
    {
        string msg = "addmultisigaddress <nrequired> <'[\"key\",\"key\"]'> [account]\n"
            ...
            "each key is a Sector address or hex-encoded public key\n"
    ...
    return CsectorAddress(innerID).ToString();
}
...
Value createmultisig(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2)
    {
        string msg = "createmultisig nrequired [\"key\",...]\n"
            ...
            "2. \"keys\" (string, required) A json array of keys which are Sector addresses or hex-encoded public keys\n"
            " [\n"
            " \"key\" (string) Sector address or hex-encoded public key\n"
            " ,...\n"
            " ]\n"
            ...
            "Sectord createmultisig 2 \"[\\\"PCHAhUGKiFKDHKW8Pgw3qrp2vMfhwWjuCo\\\",\\\"PJrhyo8CUvFZQT8j67Expre2PYLhavnHXb\\\"]\""
            "\nAs a json rpc call\n"
    ...
    CScript inner = _createmultisig(params);
    CScriptID innerID = inner.GetID();
    CsectorAddress address(innerID);
...
Value listunspent(const Array& params, bool fHelp)
{
    ...
    if (params.size() > 2)
    {
        ...
        {
            CsectorAddress address(input.get_str());
            if (!address.IsValid())
                throw JSONRPCError(-5, string("Invalid sector address: ")+input.get_str());
            if (setAddress.count(address))
    ...
    BOOST_FOREACH(const COutput& out, vecOutputs)
    {
        ...
        {
            entry.push_back(Pair("address", CsectorAddress(address).ToString()));
...
Value gettxout(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
        throw runtime_error(
            ...
            " \"addresses\" : [ (array of string) array of sector addresses\n"
            " \"sectoraddress\" (string) sector address\n"
            " ,...\n"
            " ]\n"
            " },\n"
...
string HTTPPost(const string& strMsg, const map<string,string>& mapRequestHeaders)
{
    ostringstream s;
    s << "POST / HTTP/1.1\r\n"
      << "User-Agent: Sector-json-rpc/" << FormatFullVersion() << "\r\n"
      << "Host: 127.0.0.1\r\n"
...
//
// JSON-RPC protocol.  sector speaks version 1.0 for maximum compatibility,
...
void ThreadRPCServer2(void* parg)
{
    ...
    if (mapArgs["-rpcpassword"] == "")
    {
        ...
        string strWhatAmI = "To use Sector(Sectord)";
